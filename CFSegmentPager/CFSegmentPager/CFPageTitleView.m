...
@implementation CFPageTitleView
...
- (void)titleButtonClicked:(UIButton *)button {
    ...
    CGFloat sliderWidth          = button.titleLabel.font.pointSize * (title.length+5);
...
//    }];
    self.sliderView.frame = CGRectMake(button.frame.origin.x+(button.frame.size.width-sliderWidth)/2, self.frame.size.height-2, sliderWidth, 2);
...
- (void)drawRect:(CGRect)rect {
    ...
    [self makelineStartPoint:self.sliderView.frame.origin.y + self.sliderView.frame.size.height andEndPoint:self.sliderView.frame.origin.y + self.sliderView.frame.size.height];
    [self makelineStartPoint:self.selectedButton.frame.origin.y andEndPoint:self.selectedButton.frame.origin.y];
}
